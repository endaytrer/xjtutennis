...
function OtpForm({ confirm, cancel, phone }: { confirm: (otp: string) => void, cancel: () => void, phone: string }) {
  const [otp, setOtp] = useState("");
  return <div className="flex flex-col gap-8 justify-between bg-zinc-100 dark:bg-zinc-900 border-solid border border-zinc-300 dark:border-zinc-700 shadow-lg rounded-xl p-6 w-96 max-w-full" >
    <h1 className="font-bold text-xl">MFA Verification</h1>
    <p className="text-sm text-zinc-600 dark:text-zinc-400">An OTP has been sent to your phone {phone}. Please enter it below to complete registration.</p>
    <form onSubmit={(e) => {
      e.preventDefault();
      if (otp.length > 0) confirm(otp);
    }}>
      <div className="flex flex-col gap-2">
        <label htmlFor="mfa-otp" className="mb-0 uppercase text-xs tracking-wider text-zinc-500 font-semibold">OTP Code</label>
        <input id="mfa-otp" type="text" required pattern="[0-9]+" maxLength={6} className="p-2 rounded-md outline-none border-2 border-transparent focus:border-blue-400 bg-zinc-50 dark:bg-zinc-700 text-center text-xl font-bold tracking-widest" value={otp} onChange={(e) => setOtp(e.target.value)} />
      </div>
      <div className="flex gap-2 w-full mt-6">
        <input type="button" className="flex-1 bg-gray-500 text-white rounded-md py-2 cursor-pointer" value="Cancel" onClick={cancel} />
        <input type="submit" className="flex-1 bg-blue-400 dark:bg-blue-500 text-white rounded-md py-2 cursor-pointer" value="Verify & Sign Up" />
      </div>
    </form>
  </div>
}

export async function otpPrompt(phone: string): Promise<string | undefined> {
  const dialogContainer = document.getElementById("dialog-container");
  if (dialogContainer === null) {
    throw new Error("no dialog component find")
  }
  ...
  return new Promise((res) => {
    dialogRoot.render(<OtpForm phone={phone} confirm={(otp) => {
      dialogRoot.unmount()
      dialogContainer.classList.add("hidden");
      dialogContainer.classList.remove("flex");
      res(otp);
