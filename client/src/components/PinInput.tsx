...
import "./pininput.css"
export default function PinInput({ show, value, onChange, digitClassName }: { show: boolean, value: string, onChange: (newVal: string) => void, digitClassName?: string }) {
  ...
  >

    {
      data.split("").map((val, idx) => <div key={idx}
        className={`pin-digit p-1 rounded-md outline-none border-2 border-transparent text-xl flex items-center justify-center w-8 h-10 ${digitClassName || "bg-gray-50 dark:bg-slate-600"} ${cursor === idx ? "current" : ""}`}
        style={{
